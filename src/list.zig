...
pub fn run(ctx: *Context, config: Setting) !void {
    const zig_string = try root.readFile(ctx.allocator, ctx.io, root.zig_versions);
...
pub fn runLocal(ctx: *Context, config: Setting) !void {
...
fn printList(writer: *Io.Writer, setting: Setting, zig: json.ObjectMap) !void {
    try printHeader(writer);
    try printSeparator(writer);

    // Skip master version.
    for (zig.keys()[1..]) |version| {
        const status = getStatus(setting, version);
        try printVersion(writer, .{ version, status, "", "" });
    }
    ...
    const master_key = "master";
    {
        ...
        const symbol = getStatus(setting, version);
        try printVersion(writer, .{ master_key, symbol, "(remote)", version });
    }
    try printSeparator(writer);

    if (getMasterVersion(setting.locals)) |version| {
        const symbol = getStatus(setting, version);
...
fn getStatus(setting: Setting, version: []const u8) []const u8 {
    const symbol = if (root.containsString(setting.locals, version))
        ...
        "";
    return if (mem.eql(u8, setting.zig, version))
        using_symbol
    else
        symbol;
}
...
//
//     const config_str = try readFile(ctx.allocator, ctx.io, Setting.file_name);
//     defer ctx.allocator.free(config_str);
//
//     // It borrows the string memory.
//     var config = try json.parseFromSlice(Setting, ctx.allocator, config_str, .{});
//     defer config.deinit();
//
...
//
//     try cwd.deleteFile(io, "setting.json");
//     try cwd.deleteFile(io, "zigVersions.json");
//     try cwd.deleteFile(io, "zlsVersions.json");
// }
